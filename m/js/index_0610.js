...
function fnMove(){
    ...
    function move(e) {
        ...
        if (movePos > 0) {/*下滑*/
            ...
            this_deg = (movePos/winH)*90+"deg";//90->0
            this.this_dest_deg = "90deg";//0->90deg
            that_deg = (1-movePos*2/winH)*90+"deg";
            this.that_dest_deg = "0deg";
            this.style.webkitTransformOrigin="center bottom 0px";
            oPageList[this.prevsIndex].style.webkitTransformOrigin="center top 0px";

        } else if (movePos < 0) {/*上滑*/
            //获取下一张索引
            this.prevsIndex = index == oPageList.length - 1 ? 0 : index + 1;
            var duration = desH+movePos;
            this_deg = (-movePos/winH)*90+"deg";
            this.this_dest_deg = "90deg";
            that_deg = -Math.abs(movePos/winH)*90+"deg";
            this.that_dest_deg ="0deg";
            this.style.webkitTransformOrigin="center top 0px";
            oPageList[this.prevsIndex].style.webkitTransformOrigin="center bottom 0px";

        }
        // addClass(oPageList[this.prevsIndex],"zIndex");
        this.style.webkitTransform = "rotateX("+this_deg+")";

        oPageList[this.prevsIndex].style.webkitTransform = "rotateX("+that_deg+"deg)";
    ...
    function end(e) {
        if (this.flag) {
            ...
            this.style.webkitTransform = "rotateX("+_this.this_dest_deg+")";
            oPageList[this.prevsIndex].style.webkitTransform = "rotateX("+_this.that_dest_deg+")";
            ...
            oPageList[this.prevsIndex].addEventListener('webkitTransitionEnd', function (e) {
                ...
                // _this.style.webkitTransformOrigin = "bottom";
                _this.style.webkitTransform = "rotateX(0deg)";
                this.style.webkitTransform = "rotateX(90deg)";
